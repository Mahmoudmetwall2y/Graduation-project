...
- Frontend `/api/health` is `healthy` or `degraded` with a clear explanation.
...
3. Provision the device credentials through Serial Monitor.
4. Confirm the device appears `online` in the dashboard within 90 seconds.
5. Confirm telemetry appears: RSSI, firmware, uptime/free heap when available.
6. Keep Serial Monitor open during the demo to catch preflight or stream warnings.
