"""
=============================================================================
MODEL 3: BiLSTM ECG Arrhythmia & Heart Disease Predictor (KAGGLE VERSION)
=============================================================================
...
Usage in Kaggle:
  1. Upload datasets as zip to Kaggle
  2. Adjust the base path below if your dataset name is different
  3. Run:  python model3_bilstm_ecg_arrhythmia_kaggle.py
=============================================================================
"""
...
def get_paths(colab=False, drive_root=''):
    """
    Kaggle-specific path configuration.
    """
    # The exact path based on how Kaggle unzipped the files:
    base = Path('/kaggle/input/datasets/mahmoudmetwall22y/biltsim')
    
    # Kaggle provides /kaggle/working/ for output files that you can download
    output = Path('/kaggle/working/model3_bilstm_ecg')
...
def extract_windows_from_record(signals: np.ndarray,
    ...
    """
    # Shrink the heartbeat extraction window from 500 samples to 300 (150 before, 150 after).
    # At 360Hz, 500 samples captures 3 different heartbeats in the same window,
    # causing massive label confusion for the network.
    # 300 samples (~0.83 seconds) is more appropriate for a single beat.
    window_size = 300
    half = window_size // 2 # 150 samples before, 150 samples after
...
def build_bilstm_model(input_shape: tuple, n_classes: int) -> Model:
    """
    CNN-BiLSTM for ECG beat classification.

    Architecture:
      - SpatialDropout1D for input regularization
      - 1D Convolution feature extractor (shrinks 500 steps to 125 clean shapes)
      - 3 stacked BiLSTM layers
      - LayerNormalization for stabilizing sequence training
      - GlobalMaxPooling1D to capture the heartbeat peak wherever it is
      - Dense classifier
    """
    inp = layers.Input(shape=input_shape, name='ecg_input')
    x = layers.SpatialDropout1D(0.1)(inp)
    
    # ── Morphological Feature Extractor (CNN) ──────────────────
    # This automatically finds QRS peaks, slopes, and P-waves across the 500 steps
    # and shrinks the timeline by 4x so the BiLSTM doesn't forget the past!
    x = layers.Conv1D(64, kernel_size=15, activation='relu', padding='same')(x)
    ...
    # This solves the vanishing gradient issue! The LSTM no longer has to remember
    # the exact shape of a heartbeat 250 steps ago; this catches the highest activations.
...
def main():
    ...
    if gpus:
        ...
        print("[INFO] No GPU found. Falling back to CPU...")
    ...
    if len(X_all) == 0:
        print("[ERROR] No samples loaded. Check dataset paths in Kaggle!")
        sys.exit(1)
